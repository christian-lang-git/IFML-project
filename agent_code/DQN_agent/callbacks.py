...
def setup(self):
    ...
    #region PAPER: Algorithm 1 Line 2
    '''Quote:   Initialize action-value function Q with random weights theta'''
    #self.Q = DQN_L2(input_dims=FEATURE_DIM, fc1_dims=LAYER_1_SIZE, fc2_dims=LAYER_2_SIZE, n_actions=NUM_ACTIONS).to(self.device)
    self.Q = Create_DQN(model_architecture=MODEL_ARCHITECTURE).to(self.device)
    #endregion
...
def act(self, game_state: dict) -> str:
    ...
    #region PAPER: Algorithm 1 Line 8
    '''Quote:   With probability epsilon select a random action a_t'''
    #Exploration (can occur only during training)
    ...
    #region PAPER: Algorithm 1 Line 9
    '''Quote:   otherwise select a_t = argmax Q(phi(s_t), a; theta)'''
    #Exploitation
    ...
    actions = self.Q(feature_tensor)
    action_index = T.argmax(actions)
